...
Let’s extend the **FlightItinerariesApp API** to include aircrafts details.
...
5)	Now let’s configure the Invoke Rest Service activity.  
  ...
  `https://integration.cloud.tibcoapps.com:443/wz3dr4zotkxmqg5au6yhc526iw2otlc2/v1/aircrafts/{aircraftid}`
    4)  Now let’s map activity input.  Select **Input**, expand pathParams and select aircraftid.   Expand Upstream Output->Reservation->responseBody->reservation and select aircraftid.
    5) Configure the Output settings.  Select **Output Settings** and paste the following schema into Response Schema.
```
{"aircraft":
    {"aircraftid":"111",
    "type":"767",
...
6) Let’s map the aircraft details into the Return activity.  
    ...
    3) In upstream Output Aircraft, expand Aircraft->responseBody, select aircraft.
    4) Click Save
